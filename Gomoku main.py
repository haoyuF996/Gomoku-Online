...
def Start():
    ...
    global screen
    if Won:
        pygame.draw.rect(screen, (0,0,0), Rect((SIZE[0]*50-200,0),(200,100)))
        x, y = pygame.mouse.get_pos()
...
s = socket.socket()
host,port = socket.gethostname(),23333
...
#游戏主循环
    ...
    M = GetMouse(Borad)
    #获得鼠标位置
    if not Won and M and Borad[M]<=0 and player==2:
        Borad[M] = -player
    #在鼠标所在位置展示棋子
    for event in pygame.event.get():
        ...
        if event.type == MOUSEBUTTONDOWN and OnS and Won:
            ...
            Won = False
            #点击Start后初始化
        if event.type == MOUSEBUTTONDOWN and M and Borad[M]<=0 and not Won and player==2:
            Borad[M] = player
            loc.append(M)
            sendmsg = True
    ...
    ShoeState()#展示回合状态
    DisplyBorad(Borad)
    if Borad!=tempBorad and player==2: #and sendmsg:
        #c.send(str(loc).encode())
        c.send(str(Borad).encode())
        print('sent')
    if sendmsg:
        sendmsg = False
        time.sleep(0.1)
